...
double change(Token_stream& ts, string name);
double expression(Token_stream& ts);
vector<Variable> names;
double sqrt_calc(Token_stream& ts);
double pow_calc(Token_stream& ts);
void clean_up_mess();
Token_stream ts;
...
Token Token_stream::get()
{
	...
	default:
		if (isalpha(ch))		//если буква
		{
			...
			if (s == "sqrt") return Token(number, sqrt_calc(ts));
			if (s == "pow") return Token(number, pow_calc(ts));
			if (s == "help")
			{
				help();
				return Token(print);
...
double sqrt_calc(Token_stream& ts)		//вычисляет квадрантый корень
{
...
double pow_calc(Token_stream& ts)		//возводит число в степень
{
	double val;
	Token t = ts.get();
	if (t.kind != '(')
...
void set_value(Token_stream& ts, string s, double d)		//присваивает новое значение переменной в векторе names
{
	for (int i = 0; i < names.size(); ++i)
		if (names[i].name == s)
		{
...
double primary(Token_stream& ts)		//работает со скобками
{
	Token t = ts.get();
	switch (t.kind) {
	case '(':
...
double term(Token_stream& ts)		//работает с * и /
{
	double left = primary(ts);
	while (true) {
		Token t = ts.get();
...
double expression(Token_stream& ts)		//работает с + и -
{
	double left = term(ts);
	while (true)
	{
...
double declaration(Token_stream& ts, bool f)		//объявление переменной "let имя = число"
{
	Token t = ts.get();
...
double change(Token_stream& ts, string name)		//для изменения значения переменной
{
...
double statement(Token_stream& ts)
{
	...
	{
	case let:
		return declaration(ts, 0);		//переменная
	case letc:
		return declaration(ts, 1);		//константа
	default:
		ts.unget(t);
