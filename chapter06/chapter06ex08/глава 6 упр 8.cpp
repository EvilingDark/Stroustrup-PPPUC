// глава 6 упр 8.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int main()
{
	...
	try
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			secret.push_back(randint(26) + 97);
			cout << " " << secret[i];
		}
		cout << "\n";
		int check = 0;
		while (check == 0)
		{
			...
			if (check == 1)
			{
				secret.erase(secret.begin(), secret.end());
				int number = 0;
				cout << "Введите число для создания угадываемой последовательности\n";
				cin >> number;
				srand(number);
