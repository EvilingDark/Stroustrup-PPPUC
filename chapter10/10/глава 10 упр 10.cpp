// глава 10 упр 10.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
// НЕ СДЕЛАНО
...
Token Token_stream::get()
{
	...
	default:
		if (isalpha(ch))		//если буква
		{
			string s;
			s += ch;
			while (cin.get(ch) && (isalpha(ch) || isdigit(ch) || (ch == '_') || ch == '.')) s += ch;		//считывает символ символ с потока cin и проверяет является он буквой или цифрой
			cin.unget();
...
void read_from_file()
{
	...
	if (iname.kind == name)
	{
		...
		error("calculate_from_file ожидалось название файла");
}
