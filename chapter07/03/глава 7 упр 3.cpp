// глава 7 упр 3.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
	...
	for (int i = 0; i < names.size(); ++i)
		if (names[i].name == s)
		{
			if (names[i].type == 0)
				...
				if (names[i].type == 1)
					error("Изменение константы запрещено.");
		}
	return false;
}
...
double declaration(bool f)		//объявление переменной "let имя = число"
{
	...
	double d = expression();
	if (f == 1)
		names.push_back(Variable(name, d, 1));
	else
		names.push_back(Variable(name, d, 0));
	return d;
}
